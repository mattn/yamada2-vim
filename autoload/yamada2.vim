...
function! yamada2#Yamada()
  ...
  for f in sort(split(glob(s:data_dir . '/*.xpm'), "\n"))
    ...
    for line in lines[pos1+2:pos2-1]
      ...
      if s[1] == 'NONE'
        call add(colors, printf('syntax match yamadaNONE /%s/', join(map(split(s[0], '\zs'), 'printf("[\\x%02x]",char2nr(v:val))'), '')))
        highlight yamadaNONE guifg=bg guibg=NONE ctermfg=NONE ctermbg=NONE
      else
        call add(colors, printf('syntax match yamada%s /%s/ contains=yamadaNONE', s[1][1:], join(map(split(s[0], '\zs'), 'printf("[\\x%02x]",char2nr(v:val))'), '')))
